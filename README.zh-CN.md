...
| **Command Code** | ✅ 自动 | 被动读取会话日志（`~/.commandcode/projects/<cwd-slug>/<session-id>.jsonl`，跳过 `*.checkpoints.jsonl` 快照）。统计 token 与 CLI 自己计费的 `costUsd`，并先把已含缓存读取的 input 扣回，避免重复计数；记录按字段切片读取，绝不读取 prompt、回复与代码。可用 `TOKENTRACKER_COMMANDCODE_HOME` 覆盖目录 |
