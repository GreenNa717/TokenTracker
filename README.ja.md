...
| **Command Code** | ✅ 自動 | パッシブなセッションログリーダー (`~/.commandcode/projects/<cwd-slug>/<session-id>.jsonl`、`*.checkpoints.jsonl` スナップショットはスキップ)。トークン数と CLI 自身が請求した `costUsd` を集計し、キャッシュ読み込みを含む input は差し引いて二重計上を防ぎます。フィールド単位で切り出すため、プロンプト・応答・コードは一切読みません。ディレクトリは `TOKENTRACKER_COMMANDCODE_HOME` で上書き可能 |
