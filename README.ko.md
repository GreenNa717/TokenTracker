...
| **Command Code** | ✅ 자동 | 패시브 세션 로그 리더 (`~/.commandcode/projects/<cwd-slug>/<session-id>.jsonl`, `*.checkpoints.jsonl` 스냅샷은 건너뜀). 토큰 수와 CLI가 실제 청구한 `costUsd`를 집계하며, 캐시 읽기를 포함한 input은 다시 차감해 이중 계산을 막습니다. 필드 단위로 잘라 읽기 때문에 프롬프트·응답·코드는 전혀 읽지 않습니다. 디렉터리는 `TOKENTRACKER_COMMANDCODE_HOME`으로 변경 가능 |
