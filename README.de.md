...
| **Command Code** | ✅ Auto | Passiver Session-Log-Reader (`~/.commandcode/projects/<cwd-slug>/<session-id>.jsonl`; `*.checkpoints.jsonl`-Snapshots werden übersprungen). Zählt Tokens und das vom CLI selbst abgerechnete `costUsd` und rechnet den cache-inklusiven Input wieder heraus. Datensätze werden feldweise gelesen — Prompts, Antworten und Code werden nie gelesen. Verzeichnis per `TOKENTRACKER_COMMANDCODE_HOME` überschreibbar |
